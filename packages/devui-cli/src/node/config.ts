...
export type configFile = `rabbit.comfig.${string}`
...
export type ResolvedConfig = Readonly<Omit<UserConfig,
  'plugins' | 'alias' | 'dedupe'> & {
...
export async function loadConfigFromFile(
  ...
  if (configFile) {
    ...
    // otherwise from cwd
    const jsconfigFile = path.resolve(configRoot, 'rabbit.config.js')
    if (fs.existsSync(jsconfigFile)) {
      resolvedPath = jsconfigFile
    }

    if (!resolvedPath) {
      const mjsconfigFile = path.resolve(configRoot, 'rabbit.config.mjs')
    ...
    if (!resolvedPath) {
      const tsconfigFile = path.resolve(configRoot, 'rabbit.config.ts')
    ...
    if (!resolvedPath) {
      const cjsConfigFile = path.resolve(configRoot, 'rabbit.config.cjs')
